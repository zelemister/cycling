...
from xaifunctions import explainme
...
resolution = 256
image_transformation = get_transformer(transformation_name, resolution)
...
for file in os.listdir(folder_path):
    ...
    if file.endswith(".png") or file.endswith(".jpg"):
        original_image = Image.open(os.path.join(folder_path, file))
        images_original.append(original_image)
       # apply a xai function yet to be determined
        lrp_image = explainme(original_image)
        images_explained.append(lrp_image)
       # test with transforms
        transformed_image = image_transformation(original_image)  # outputs a tensor
        images_transformed.append(transforms.ToPILImage()(transformed_image))
        transformed_image = transformed_image.permute(1, 2, 0)
        transformed_image = transformed_image.to(dtype=torch.uint8)  # unsigned 8bit integers range from 0-255
...
for ax, (image1, image2, image3) in zip(axs, zip(images_original, images_transformed, images_explained)):
    ax[0].imshow(image1)
    ax[1].imshow(image2)
    ax[2].imshow(image3)
    ax[0].axis('off')
